...
export class LdapService {
  ...
  async authenticate(username: string, password: string): Promise<LdapUser> {
    ...
    try {
      ...
      if (isDev) {
        this.logger.warn(`LDAP falló — fallback de desarrollo: ${(err as Error).message}`)
        return this.devFallback(username, password)
      }

      // Producción: cualquier error de infraestructura LDAP = 401 (no exponer detalles)
      this.logger.error('Error de infraestructura LDAP', (err as Error).message)
  ...
  private ldapBind(username: string, password: string): Promise<LdapUser> {
    return new Promise((resolve, reject) => {
      ...
      if (!url || !bindDn || !baseDn) {
        return reject(new Error('LDAP no configurado (LDAP_URL, LDAP_BIND_DN o LDAP_BASE_DN ausentes)'))
      }

      const client = ldap.createClient({ url, reconnect: false, timeout: 5000, connectTimeout: 4000 })
      ...
      client.bind(bindDn, bindPw, (bindErr) => {
        if (bindErr) {
          client.destroy()
          return reject(new Error(`Bind de cuenta de servicio falló: ${bindErr.message}`))
        }
        ...
        client.search(baseDn, searchOpts, (searchErr, res) => {
          if (searchErr) {
            client.destroy()
            return reject(new Error(`Búsqueda LDAP falló: ${searchErr.message}`))
          }
          ...
          res.on('end', () => {
            ...
            try {
              ...
              const email: string = String(obj['mail'] ?? `${username}@farmcorp.com.ec`)
              ...
              if (!userDn) {
                client.destroy()
                return reject(new UnauthorizedException('No se pudo obtener el DN del usuario'))
              }
