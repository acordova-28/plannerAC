...
export class CamposService {
  constructor(
    ...
    @InjectRepository(TaskDinamico) private readonly dinamicoRepo: Repository<TaskDinamico>,
  ) {}
  ...
  async setValor(taskId: string, campoId: string, dto: SetValorDto): Promise<TaskDinamico> {
    let dinamico = await this.dinamicoRepo.findOneBy({ taskId, campoId })
    if (!dinamico) {
      dinamico = this.dinamicoRepo.create({ taskId, campoId, valor: dto.valor })
    } else {
  ...
  getValoresByTask(taskId: string): Promise<TaskDinamico[]> {
    return this.dinamicoRepo.find({ where: { taskId }, relations: { campo: true } })
  }
}
