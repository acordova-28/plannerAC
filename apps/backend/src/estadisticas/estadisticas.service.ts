...
export class EstadisticasService {
  ...
  async getEstadisticas(): Promise<EstadisticasDto> {
    ...
    const horasEstimadas = tasks.reduce((s, t) => s + (t.horasEstimadas ?? 0), 0)
    ...
    for (const t of tasks) {
      ...
      const responsables = t.responsables?.map(r => r.user?.nombre).filter(Boolean) ?? []
    ...
    const porcentajeAvance = totalTareas ? Math.round((completadas / totalTareas) * 100) : 0

    return { totalTareas, horasEstimadas, horasBaseline, porEstado, porModulo, porResponsable, porcentajeAvance }
  }
}
